...
const PlotlineDrawer = ({ open, close, isUpdate, onSubmit, plotlineId }) => {
  const { id } = useParams();
  ...
  const updateData =
    plotlines?.length > 0 &&
    plotlines?.filter((item) => item?.id === plotlineId);

  const plotlineName = updateData[0]?.plot_planner_title;
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      plot_planner_title: plotlineName,
  ...
  return (
    <Drawer
      title={isUpdate ? "Update PlotLine" : "Create New Plotline"}
      placement="right"
      onClose={close}
      open={open}
    ...
    >
      <Form onFinish={handleSubmit} form={form}>
        ...
        <div className="artworkContainer mt-5">
          <Dropdown
            trigger={"click"}
            overlay={<GalleryList
              gallery={gallery}
        ...
        <Form.Item
          ...
          rules={[
            {
              required: true,
              message: "Plotline name is required!",
            },
          ]}
        >
          <Input placeholder="Enter Ploline Name" />
        </Form.Item>
